...
function ClientJoinPage() {
  ...
  useEffect(() => {
    //promotion 키로 부터 받아온 정보 입력
    if (validatedClient?.promotionKey) {
      setPromotionKey(validatedClient.promotionKey);
    }
    if (validatedClient?.email) {
      setEmail(validatedClient.email);
    }
    if (validatedClient?.name) {
      setName(validatedClient.name);
    }
    if (validatedClient?.phone) {
      setPhone(validatedClient.phone);
    }
